...
Test Execution
      ↓
toKarateJson()  ← Single canonical format
      ↓
  ┌───┴───┐
  ↓       ↓
JSONL   HTML Reports
  ↓
Cucumber JSON (post-process)
```

**Key principles:**
- **Single source of truth**: `toKarateJson()` methods on result classes
- **Cucumber JSON from JSONL**: Generated by post-processing, not directly from runtime
- **HTML reports use karate JSON**: Inlined as JSON in HTML for client-side rendering
...
| `CucumberJsonConverter` | Converts karate JSONL → cucumber JSON |
...
// Generate Cucumber JSON (from JSONL post-processing)
Runner.path("features/")
    .outputJsonLines(true)
...
Cucumber JSON is generated by converting `FEATURE_EXIT` events from `karate-events.jsonl`. The format is **final and must not change** for compatibility with third-party tools.
...
karate-events.jsonl
        ↓
Filter FEATURE_EXIT events (contains full toKarateJson())
        ↓
...
| `CucumberJsonConverterTest` | JSONL → Cucumber JSON conversion |
